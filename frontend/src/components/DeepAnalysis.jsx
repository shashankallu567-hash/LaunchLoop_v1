...
export default function DeepAnalysis({ headline, body, audience, goal, platform, heuristic, testid = "deep-analysis" }) {
  ...
  const run = async () => {
    setState("loading");
    try {
      const { data } = await api.post("/deep-analysis", { headline, body, audience, goal, platform, heuristic });
      setRes(data);
      setState("done");
    } catch {
      setState("error");
  ...
  return (
    <div className="rounded-xl border border-border bg-panel p-6" data-testid={testid}>
      ...
      <p className="text-xs text-muted-foreground mb-4">Optional — the heuristic stays your primary score. AI independently judges the same five factors.</p>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} data-testid="deep-analysis-result">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 mb-5">
              ...
              <div className="rounded-lg border border-growth/30 bg-growth/5 p-4 text-center">
                ...
                <CountUp value={res.ai.overall} className="font-mono-metric text-3xl font-bold" style={{ color: "var(--score-strong)" }} />
              </div>
            </div>
