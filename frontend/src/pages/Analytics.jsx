...
function chartColors(theme) {
  ...
  return {
    ...
    polar: dark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)",
    axis: dark ? "#A7B0BC" : "#59616B",
    tooltipBg: dark ? "#171C22" : "#FFFFFF",
    tooltipBorder: dark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)",
    lime: dark ? "#B7FF00" : "#79A900",
    info: dark ? "#4C8DFF" : "#2563EB",
    muted: dark ? "#A7B0BC" : "#59616B",
    barMuted: dark ? "#3a444f" : "#6F7A6B",
  };
}

function scoreColor(score, c) {
  return score >= 80 ? c.lime : score >= 65 ? c.info : c.muted;
}
...
export default function Analytics() {
  ...
  const tooltipStyle = { background: c.tooltipBg, border: `1px solid ${c.tooltipBorder}`, borderRadius: 8, fontSize: 12, color: "hsl(var(--foreground))" };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-dashed border-border bg-panel p-12 text-center">
          <BarChart3 className="mx-auto text-foreground/30" size={30} />
          <p className="text-sm text-muted-foreground mt-3">Run your first launch to unlock analytics.</p>
          <button onClick={() => navigate("/app/launch")} className="mt-4 inline-flex items-center gap-2 bg-foreground text-background font-semibold px-4 py-2 rounded-lg text-sm"><Zap size={16} /> Create Launch</button>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="rounded-xl border border-border bg-panel p-5" data-testid={`analytics-stat-${s.label.toLowerCase().replace(/\s/g, "-")}`}>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-xl border border-border bg-panel p-6 flex items-start gap-4" data-testid="best-angle-card">
        ...
        <div className="rounded-xl border border-border bg-panel p-6 flex items-start gap-4" data-testid="best-platform-card">
          <div className="h-10 w-10 rounded-lg bg-neutral/10 flex items-center justify-center"><Gauge size={20} style={{ color: c.info }} /></div>
          <div>
        ...
        <Panel title="Virality score trend" testid="chart-score-trend">
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={data.score_trend}>
              ...
              <Line type="monotone" dataKey="score" stroke={c.lime} strokeWidth={2.5} dot={{ r: 3, fill: c.lime }} />
            </LineChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="Prediction accuracy over time" testid="chart-accuracy">
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={data.accuracy_trend}>
              ...
              <YAxis domain={[0, 100]} tick={AXIS} axisLine={false} tickLine={false} width={28} />
              <Tooltip contentStyle={tooltipStyle} />
              <Line type="monotone" dataKey="accuracy" stroke={c.info} strokeWidth={2.5} dot={{ r: 3, fill: c.info }} />
            </LineChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="Predicted vs actual impressions" testid="chart-pred-real">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={data.pred_vs_real}>
              ...
              <Tooltip contentStyle={tooltipStyle} />
              <Legend wrapperStyle={{ fontSize: 12 }} formatter={(value) => <span style={{ color: c.axis }}>{value}</span>} />
              <Bar dataKey="predicted" fill={c.barMuted} radius={[3, 3, 0, 0]} />
              <Bar dataKey="actual" fill={c.lime} radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="Average score by factor" testid="chart-factors">
          <ResponsiveContainer width="100%" height={240}>
            <RadarChart data={data.factor_avg}>
              ...
              <PolarAngleAxis dataKey="factor" tick={{ fontSize: 11, fill: c.muted }} />
              <PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} />
              <Radar dataKey="value" stroke={c.lime} fill={c.lime} fillOpacity={0.25} strokeWidth={2} />
              <Tooltip contentStyle={tooltipStyle} />
            </RadarChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <div className="space-y-2">
          {data.top_angles.map((a, i) => (
            <button key={a.id} onClick={() => navigate(`/app/experiments/${a.id}`)} data-testid={`top-angle-${a.id}`}
              className="w-full text-left flex items-center justify-between gap-4 rounded-lg border border-border bg-surface px-4 py-3 hover:border-borderStrong transition-colors">
              ...
              <span className="font-mono-metric font-bold" style={{ color: scoreColor(a.score, c) }}>{a.score}</span>
            </button>
          ))}
        </div>
      </Panel>
