...
const STAGES = [
  { key: "prediction", label: "PREDICTION", icon: Target, color: "#0066FF" },
  { key: "launch", label: "LAUNCH", icon: Rocket, color: "#FFFFFF" },
  { key: "reality", label: "REALITY", icon: Activity, color: "#D3FF24" },
  { key: "learning", label: "LEARNING", icon: Brain, color: "#FF3366" },
];

function StageHeader({ stage, num }) {
  return (
    <div className="flex items-center gap-3">
      <div className="h-9 w-9 rounded-lg border border-white/10 bg-surface flex items-center justify-center">
...
export default function ExperimentDetail() {
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => navigate("/app/experiments")} data-testid="back-experiments" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-white transition-colors">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button onClick={() => navigate(`/report/${c.id}`)} data-testid="open-report-btn"
          className="flex items-center gap-2 border border-white/15 px-4 py-2.5 rounded-lg text-sm hover:bg-surface transition-colors">
      ...
      <div className="grid md:grid-cols-4 gap-3">
        {STAGES.map((s, i) => (
          <div key={s.key} className="rounded-lg border border-white/10 bg-panel p-3 flex items-center gap-2">
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        ...
        <div className="rounded-xl border border-white/10 bg-panel p-6 flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">
            Launched to <span className="text-white">{(c.platforms || [c.angle.channel]).join(", ")}</span> targeting <span className="text-white">{c.audience_snapshot?.name || "your audience"}</span>.
          </div>
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={() => setEditing((v) => !v)} data-testid="enter-real-btn"
            className="flex items-center gap-2 border border-white/15 px-4 py-2 rounded-lg text-sm hover:bg-surface transition-colors">
          ...
          <button onClick={resynth} disabled={busy} data-testid="resynth-btn"
            className="flex items-center gap-2 border border-white/15 px-4 py-2 rounded-lg text-sm hover:bg-surface transition-colors disabled:opacity-60">
        ...
        {editing && (
          <div className="rounded-xl border border-white/10 bg-panel p-6" data-testid="real-metrics-form">
              ...
              {["impressions", "engagement", "shares", "conversions"].map((k) => (
                <div key={k}>
                  ...
                  <input type="number" value={form[k]} onChange={(e) => setForm({ ...form, [k]: e.target.value })} data-testid={`real-${k}`}
                    className="mt-1.5 w-full bg-surface border border-white/10 rounded-lg px-3 py-2 text-sm font-mono-metric focus:outline-none focus:ring-2 focus:ring-white/50" />
                </div>
              ))}
            </div>
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        ...
        <div className="rounded-xl border border-white/10 bg-panel p-6 grain" data-testid="learning-note">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button onClick={() => navigate("/app/launch")} data-testid="start-new-experiment"
              className="flex items-center gap-2 bg-white text-black font-semibold px-5 py-2.5 rounded-lg text-sm transition-transform hover:scale-[0.98] active:scale-95">
              <Rocket size={16} /> Start new experiment
            </button>
            <button onClick={() => navigate(`/report/${c.id}`)} className="flex items-center gap-2 border border-white/15 px-5 py-2.5 rounded-lg text-sm hover:bg-surface transition-colors">
              <Share2 size={16} /> Share report
            </button>
          </div>
        </div>
